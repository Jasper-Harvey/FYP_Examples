function best_pose = ICP_SVD_SE2(P1, P2, iterations)

% P1 static points
% P2 moving points

T = eye(3); % Initial homogeneous transform
iters = 200;
min_err = 1e-4;
min_iters = 5;
err = 0;
...
%     pair_moving = movingP(idx, 1:3);
    ...
    pair_static = P1_knn(idx ~= -1, 1:2);
    idx = idx(idx ~= -1);
    pair_moving = P2_knn(idx, 1:2);
...
%     tempt = mean(pair_static,1).' - (tempR*mean(pair_moving,1).');
    tempt = mean(staticP(:,1:2),1).' - (tempR*mean(movingP(:,1:2),1).');
%     tempt = mean(pair_moving).' - (tempR*mean(pair_static).');

    newT = eye(3);
    newT(1:2,1:2) = tempR(1:2,1:2);
    newT(1:2,3) = tempt;
